...
> 当前版本已移除陀螺仪软件优化流水线，直接显示驱动换算后的原始物理量；姿态角行固定显示 `---`（算法待重构）。
...
| 3 | `A 0.12 -0.05 0.99` | 三轴加速度 X / Y / Z | MPU 加速度计（驱动换算原始值） | g（重力倍数） | 固定宽 2 位小数 |
| 4 | `G 12.34 46.78 -5.12` | 三轴角速度 X / Y / Z | MPU 陀螺仪（原始值，无软件优化） | °/s | 固定宽 2 位小数 |
| 5 | `M 45.12 -23.34 12.56` | 三轴磁场 X / Y / Z | AK8963 | µT | 固定宽 2 位小数 |
| 6 | `R---P---Y---` | 姿态角占位（算法已移除） | — | ° | 固定宽占位 |
| 7 | `Up:00:05:23 Heap:123K` | 运行时间 + 空闲堆 | 系统 | 时:分:秒 / KB | — |

补充说明：

- **A（加速度）**：静止时 Z 轴约 `1.00`（重力），X/Y 接近 0；数值为 g，不是 m/s²。驱动仅做 LSB→g 换算，无软件滤波。
- **G（陀螺仪）**：驱动仅做 LSB→°/s 换算；**未做零偏补偿/滤波/ZUPT**，静止时可能有可见偏置与噪声。
- **M（磁力计）**：µT；三轴全为 0（无磁力计或读取失败）时整行显示 `---`。
- **R/P/Y**：姿态融合算法已从构建中移除，行固定为 `R---P---Y---`，待重构后恢复。
...
9. **IMU 数据路径（当前为原始数据基线）**
   ...
   - `mpu9250`：
     ...
     - `0x3B~0x48` 一次突发读 14 字节（加速度+温度+陀螺），保证同帧一致。
   - 显示不做零偏补偿、软件滤波、ZUPT 或姿态融合；G 行会看到真实偏置与噪声。
   - 姿态角行固定 `R---P---Y---`，等新算法接入后替换。
   - 陀螺量程 **±500dps**（`FS_SEL=01`，65.5 LSB/°/s）。IMU 独立任务 100Hz（`IMU_PERIOD_MS = 10`）。
