...
> 当前版本：**A/G/M 显示驱动原始值**；**仅 R/P/Y 走 imu_algo**（滤波 → 零偏 → Mahony 六轴姿态）。
...
| 3 | `A 0.12 -0.05 0.99` | 三轴加速度 X / Y / Z | MPU 驱动原始值（仅硬件 DLPF） | g（重力倍数） | 固定宽 2 位小数 |
| 4 | `G 12.34 46.78 -5.12` | 三轴角速度 X / Y / Z | MPU 驱动原始值（无软件滤波/零偏） | °/s | 固定宽 2 位小数 |
...
- **A（加速度）**：驱动换算原始值；静止时 Z 约 `1.00`，X/Y 接近 0，未做软件零偏。
- **G（陀螺仪）**：驱动换算原始值；静止时仍可能有可见零偏与噪声，**不上屏优化结果**。
- **M（磁力计）**：µT；三轴全为 0（无磁力计或读取失败）时整行显示 `---`。
- **R/P/Y**：唯一走软件优化链的输出；标准 ZYX，固定宽度 2 位小数。算法未就绪时显示 `R---P---Y---`。
- 所有数值右对齐到固定宽度，小数点位置固定；无效数据显示为固定宽度的 `---`。
- 数值超出字段宽度时（如陀螺仪超过 ±999.99），该行会临时变宽、末位可能被裁掉。
- 传感器离线时对应行显示 `---`，程序不会卡死。
- 环境传感器每 50ms 采集一次，IMU 以 200Hz 独立采集，显示刷新周期默认 20ms（见 `IMU_PERIOD_MS` / `DISPLAY_PERIOD_MS`）。
...
9. **IMU 数据路径（A/G 原始，姿态走算法）**
   ...
   - **姿态路径**（与显示分离，内部仍优化）：
     ...
     同一帧 raw → imu_filter（二阶 biquad 低通）
               ...
               → 输出 R/P/Y
     ```
   - **时序（200Hz）**：上电 delay 1000 帧 ≈5s 陀螺预热（不积分）→ 静止校准 400 帧 ≈2s
   ...
   - **零偏校准**：按 `imu_bias_calib.c` 中 `CALIB_DELAY_SAMPLE_NUM` / `CALIB_SAMPLE_NUM`
     执行（1000+400 帧 @200Hz ≈5s+2s），完成后串口打印 `IMU 零偏校准完成`。
     校准只影响姿态，**不影响 A/G 上屏数值**。
   - **OLED 姿态行**：预热/校准中显示进度条（`WARM[###...] 42%` / `CAL [...]`），
     完成后显示 R/P/Y。
...
4. **姿态角校准**
   - 上电后请保持水平静止约 7s（预热约 5s + 校准约 2s @200Hz）。
   - 预热与校准期间 OLED 第 6 行显示进度条（`WARM` / `CAL`），不做姿态积分；
     串口出现 `IMU 零偏校准完成` 后，R/P/Y 以当时姿态为原点归零并开始输出。
